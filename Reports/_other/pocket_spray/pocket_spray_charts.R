...
opposing_hitter_pos_cards <- function(database, hitters, home_team_code) {
  ...
  get_legend <- function(my_plot) {
    ...
    leg <- gtable_filter(tmp, "guide-box-bottom")
    return(leg)
  }
  ...
  plot_leg <- ggplot(df, aes(x=x,y=y, fill = HitType))+
    geom_point(pch = 21 , color = 'black', size = 2, stroke = 1.25)+
  ...
  # POSITIONING CARDS ----------------------------------------------------------------------
  {
    ...
    for(i in seq_along(hitter_sides_lineup$Batter)) {
      ...
      if(if_bip>=100) {
        ...
        if_spray_plot <- ggplot(data = if_spray, aes(x=hc_x, y=hc_y))+ 
          ...
          theme_void()+
          theme(legend.position = 'bottom',
                plot.title = element_text(hjust = .5, color = title_col, size = 10, face = 'bold'))+
          new_scale_fill() +
          annotate(geom = 'text', x = 80, y = 15, label = glue("{if_bip}\nBIP"), size = 3)
      ...
      if(of_bip >= 100) {
        ...
        of_spray_plot <- ggplot(data = of_spray, aes(x=hc_x, y=hc_y))+
          ...
          scale_fill_manual(values = hit_colors)+
          annotate(geom = 'text', x = 200, y = 80, label = glue("{of_bip}\nBIP"), size = 3)
    ...
    plot_with_legend <<- arrangeGrob(legend, plot_of, ncol = 1, heights = c(.25, 10))
...
opp_hitters <- c('Alec Craig', 'Andrew Sojka', 'Chase Dawson', 'Anthony Calarco', 'Christian Fedko',
                 'Nick Oddo', 'Aaron Simmons', 'Kyle Fitzgerald', 'Bren Spillane', 'Will Prater',
