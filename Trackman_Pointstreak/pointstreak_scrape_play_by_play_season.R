...
seasonid <- 33873
# dates for season
game_dates <- unique(format(as.Date(unique(
  read_html(glue::glue('https://baseball.pointstreak.com/schedule.html?leagueid={leagueid}&seasonid={seasonid}')) %>%
    html_table(fill = T) %>% .[[1]] %>%
    mutate(dt = as.Date(Date, "%b %d")- lubridate::years(1)) 
  # %>%
  #   filter(dt < Sys.Date())
)$dt), 
"%m/%d/%Y")
...
site_url <- 'https://baseball.pointstreak.com/'
{
  ...
  for (date in game_dates) {
    ...
    scoreboard_url <- glue::glue('https://baseball.pointstreak.com/scoreboard.html?leagueid={leagueid}&seasonid={seasonid}&date=')
    ...
    if(length(links) > 0){
      ...
      for (i in seq_along(links)) {
        ...
        box_master <- rbind(box_master, boxscores)
  ...
  tictoc::toc()
  # dbWriteTable(db_aa, name = 'pbp_scrape_raw', value = pbp)
  pbp <- dbGetQuery(db_aa, "select * from pbp_scrape_raw")
  # CLEAN PBP DATA----
  pbp_clean <- pbp %>%
...
order by Player')
  ...
  #          RE_diff = RE_end - RE + as.numeric(RunsonPlay), .after = RE) %>%
  #   mutate(test = ifelse(InningPA == max(InningPA) & desired_result == 'out', 0.00, RE_end - RE + as.numeric(RunsonPlay))) %>%
  ...
  woba_scale_ <- dbGetQuery(db_aa, 'select woba_scale from weights where SEASON = 2024')$woba_scale
  ...
  # dbWriteTable(aa, name = 'weights_raw', value = weights, overwrite = T)
  
}

# dbWriteTable(aa, name = 'pbp_25', value = play_by_play_final, overwrite = T)
# dbWriteTable(aa, name = 'RE24', value = run_matrix, overwrite = T)
# dbWriteTable(aa, name = 'RE24_long', value = run_matrix_long, overwrite = T)
# END HERE ##################################################################################################################################################### #
