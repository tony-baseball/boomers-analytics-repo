...
r_scripts <- list(
  ...
  'catcher_framing' = 'C:/Users/tdmed/OneDrive/_Github/boomers-fl-automations/Reports/_other/catcher_framing.R'
)
...
scripts_to_run <- r_scripts[dcs]
{
  ...
  pitch_count_1 <- dbGetQuery(db, 'select count(*) pitch_count from pitch_data 
                              where SEASON = 2025 and 
                              (BatterTeam in ("Schaumburg Boomers", "SCH_BOOM") 
                                    OR PitcherTeam in ("Schaumburg Boomers", "SCH_BOOM") )')
  # pitch_count_1 <- 5
  pitch_count_1 
  ...
  pitch_count_2 <- dbGetQuery(db, 'select count(*) pitch_count from pitch_data 
                              where SEASON = 2025 and 
                              (BatterTeam in ("Schaumburg Boomers", "SCH_BOOM") 
                                    OR PitcherTeam in ("Schaumburg Boomers", "SCH_BOOM") )')
  # pitch_count_2 <- 10
